...
```
```{r echo=FALSE, Bivariate_Plots}
ggplot(data=wineData, aes(x=density, y=quality)) + geom_jitter(alpha=0.02) + scale_x_continuous(limits = c(0.95, 1.05))
```
The horizontal lines indicate quality values are all integers. After adding jitter, transparency and changing the y limits, it shows a slight correlation between density and quality(negative).

```{r echo=FALSE, Bivariate_Plots}
ggplot(data=wineData, aes(x=alcohol, y=quality)) + geom_point()
```
```{r echo=FALSE, Bivariate_Plots}
ggplot(data=wineData, aes(x=alcohol, y=quality)) + geom_jitter(alpha=0.05)
```
Similarly, After adding jitter and transparency, it shows a slight correlation between alcohol and quality(positive).
